...
abstract class ScopeFilter<N,T>(scopes: ScopeAccess) : Predicate<T>{
	...
	protected fun traverse(scopes:Array<N>, graph : N = ANY) : Iterator<N> = iterator f@{
		...
		var remaining = scopes

		// Check this and super-records (dfs)
		for(a in _super(graph)){
			remaining = check(remaining, a);
			if(remaining.isEmpty()){
				// All scopes found, yield this and sub-records (dfs), then return.
				yieldAll(_sub(graph))
				return@f
			}
		}
...
class ScopeFilterQuad(dsg:DatasetGraph, scopes: ScopeAccess) : ScopeFilter<Node, Quad>(scopes){
    ...
    override fun typedArray(vararg elements: Node): Array<Node> = arrayOf(*elements)
...
class ScopeFilterTDB2(dsg: DatasetGraph, scopes: ScopeAccess) : ScopeFilter<NodeId, Tuple<NodeId>>(scopes){
 ...
 override fun typedArray(vararg elements: NodeId): Array<NodeId> = arrayOf(*elements)

 override fun graphNodeIterator(): Iterator<Node> = Txn.calculateRead(dataset){iterator {
	val nt = TDBInternal.getDatasetGraphTDB(dataset).getQuadTable().getNodeTupleTable().getNodeTable()
	for(access in scopeIds){
